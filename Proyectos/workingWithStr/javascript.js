...
HINT 2: The solution only needs to work for a variable made out of 2 words, like 
a_b
HINT 3: Start without worrying about the ✅. Tackle that only after you have the 
variable name conversion working 😉
HINT 4: This challenge is difficult on purpose, so start watching the solution in 
...
function convertingStr(str) {
  const filas = str.split("+");
  console.log(filas);
  for (const fila of filas) {
    const [type, from, to, time] = fila.split(";");

    const resultado = `${type.startsWith("_Delayed") ? "🔴" : ""}${type
      .split("_")
      .join(" ")} ${from.slice(0, 3).toUpperCase()} ${to
      .slice(0, 3)
